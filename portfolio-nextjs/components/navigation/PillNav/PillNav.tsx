...
export default function PillNav({
  ...
  const LogoInner = <img src={logo} alt={logoAlt} ref={logoImgRef} />

  return (
    <div className="pill-nav-container">
      <nav className={`pill-nav ${className}`} aria-label="Primary" style={cssVars}>
        {logoNode ? (
          <a className="pill-logo pill-logo--node" href="#home" aria-label="Home" onMouseEnter={handleLogoEnter} ref={logoRef}>
            {logoNode}
          </a>
        ) : isRouterLink(items?.[0]?.href) ? (
          <Link className="pill-logo" href={items[0].href} aria-label="Home" onMouseEnter={handleLogoEnter} ref={logoRef}>
            {LogoInner}
          </Link>
        ) : (
        ...
        {cta ? <div className="pill-cta desktop-only">{cta}</div> : null}
        ...
        <ul className="mobile-menu-list">
          {items.map((item, i) => (
            <li key={item.href || `mobile-item-${i}`}>
              {isRouterLink(item.href) ? (
                <Link href={item.href} className={`mobile-menu-link${activeHref === item.href ? ' is-active' : ''}`} onClick={() => setIsMobileMenuOpen(false)}>
                  {item.label}
                </Link>
              ) : (
